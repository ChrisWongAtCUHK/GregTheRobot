...
class BaseLevel extends Phaser.State {
    ...
    readFile(file) {
        ...
        let pathArray = window.location.pathname.split("/").filter((x) => x.trim().length > 0);
        console.log(pathArray);
        request.open("GET", window.location.protocol + "//" + window.location.hostname +
            (window.location.port != "" ? ":" + window.location.port : "") +
            (pathArray.length > 0 ? "/" + pathArray.join("/") : "") + file, false);
        request.send(null);
